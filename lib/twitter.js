...
Twitter.prototype.updateWithMedia = function(text, image, type, callback) {
  var url = this.options.update_with_media_base + '/statuses/update_with_media.json',
      orderedParameters = this.oauth._prepareParameters(this.options.access_token_key, this.options.access_token_secret, 'POST', url, {}),
      authorization = this.oauth._buildAuthorizationHeaders(orderedParameters),
      data = {
        status: text,
        'media[]': { file: image, content_type: type }
      },
      options = {
        multipart: true,
